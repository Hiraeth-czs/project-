...
def about(subject):
    ...
    def about_words(paragraph):
        new_paragraph = split(lower(remove_punctuation(paragraph)));
        for word_in_paragraph in new_paragraph:
            if word_in_paragraph in subject:
                return True
        return False
...
def accuracy(typed, source):
    ...
    for i in range(length):
        if typed_words[i] ==source_words[i]:
            match+=1
    return 100*match/len(typed_words)
    # END PROBLEM 3
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    assert len(typed) <= len(source)
    time=0
    for i in range(len(typed)):
        if typed[i] == source[i]:
            time += 1
        else:
            upload({'id': user_id, 'progress': time/len(source)})
            return time/len(source)
...
def fastest_words(words_and_times):
    ...
    for j in range(len(word_indices)):
        ...
        for i in range(len(player_indices)):
            if get_time(times, t, j) > get_time(times, i, j):
                t=i
        fast[t].append(words[word_indices[j]])
    return fast
